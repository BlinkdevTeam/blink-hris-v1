...
function DocumentsTab() {
  ...
  const docs = [
    ...
    { name: "Payslip — Dec 2025",       type: "Payroll",  size: "42 KB",  uploaded: "Jan 1, 2026",  uploader: "Payroll Sys", status: "Auto",    ext: "pdf" },
  ];
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex gap-2 flex-wrap">{cats.map(c => <button key={c} onClick={() => setFilter(c)} className="px-3 py-1.5 rounded text-xs transition-all" style={{ fontFamily: "system-ui,sans-serif", backgroundColor: filter === c ? "#fff" : "#111", color: filter === c ? "#000" : "#666", border: filter === c ? "none" : "1px solid #2a2a2a" }}>{c}</button>)}</div>
          ...
          <tbody>
            {filtered.map((doc, i) => (
              <tr key={i} className="group cursor-pointer" style={{ borderBottom: "1px solid #141414", backgroundColor: "#0d0d0d" }} onMouseEnter={e => e.currentTarget.style.backgroundColor = "#111"} onMouseLeave={e => e.currentTarget.style.backgroundColor = "#0d0d0d"}>
                ...
                <td className="px-4 py-3"><div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button className="text-xs text-gray-400 hover:text-white">↓</button><button className="text-xs text-gray-400 hover:text-white">⋯</button></div></td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function People() {
  ...
  return (
    <div className="min-h-screen text-white flex flex-col" style={{ fontFamily: "'Georgia',serif", backgroundColor: "#000" }}>
      ...
      {view === "directory"
        ? <Directory onViewProfile={emp => { setProfileEmp(emp); setView("profile"); }} onEditEmployee={emp => setEditEmp(emp)} />
