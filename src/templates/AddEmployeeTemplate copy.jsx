...
// ── ADD EMPLOYEE DRAWER ───────────────────────────────────────────────────────
const BLANK = { name:"", email:"", role:"", dept:"Engineering", location:"New York", status:"Active", joined:"", salary:"", payFreq:"Bi-weekly", empType:"Regular", manager:"", phone:"", schedule:"Mon–Fri, 9am–5pm", benefits:"Standard", gender:"", dob:"" };
const STEPS = ["Personal Info","Role & Department","Compensation","Review"];
...
function AddEmployeeDrawer({ onClose, onSave }) {
  ...
  const canNext = [
    ...
    form.role.trim().length > 1 && form.dept,
    form.salary,
    true,
  ];
  ...
  return (
    <>
      <div className="fixed inset-0 z-20" style={{backgroundColor:"rgba(0,0,0,0.65)"}} onClick={!done ? onClose : undefined}/>
      <div className="fixed top-0 right-0 h-full z-30 flex flex-col" style={{width:480,backgroundColor:"#080808",borderLeft:"1px solid #222",boxShadow:"-8px 0 40px rgba(0,0,0,0.9)"}}>

        {/* Header */}
        <div className="flex items-center justify-between px-7 py-5 flex-shrink-0" style={{borderBottom:"1px solid #1a1a1a"}}>
          <div>
            <p className="text-xs uppercase tracking-widest text-gray-500 mb-0.5" style={{fontFamily:"system-ui,sans-serif"}}>
              {done ? "Complete" : `Step ${step+1} of ${STEPS.length} · ${STEPS[step]}`}
            </p>
            <h2 className="text-base font-normal text-white">Add New Employee</h2>
          </div>
          {!done && <button onClick={onClose} className="text-gray-600 hover:text-white text-xl">✕</button>}
        </div>

        {/* Progress bar */}
        {!done && (
          <div className="h-0.5 flex-shrink-0" style={{backgroundColor:"#1a1a1a"}}>
        ...
        {done ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center px-8 gap-4">
            ...
            <div>
              <h3 className="text-xl font-normal text-white mb-1">{form.name} added</h3>
              <p className="text-gray-500 text-sm" style={{fontFamily:"system-ui,sans-serif"}}>{form.role} · {form.dept}</p>
              <p className="text-gray-600 text-xs mt-1" style={{fontFamily:"system-ui,sans-serif"}}>{form.email}</p>
            </div>
            <button onClick={onClose} className="mt-4 px-6 py-2.5 rounded text-sm font-medium bg-white text-black hover:opacity-80" style={{fontFamily:"system-ui,sans-serif"}}>
              Done
            </button>
          </div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto px-7 py-6 space-y-5">
              ...
              {step === 0 && (
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="First Name"><input className={IC} style={IS} placeholder="e.g. Sara" value={form.name.split(" ")[0]} onChange={e=>set("name",e.target.value+" "+(form.name.split(" ").slice(1).join(" ")))}/></Field>
                    <Field label="Last Name"><input className={IC} style={IS} placeholder="e.g. Okafor" value={form.name.split(" ").slice(1).join(" ")} onChange={e=>set("name",(form.name.split(" ")[0]||"")+" "+e.target.value)}/></Field>
                  </div>
                  ...
                  <Field label="Phone Number"><input className={IC} style={IS} placeholder="+1 212 555 0000" value={form.phone} onChange={e=>set("phone",e.target.value)}/></Field>
                  <div className="grid grid-cols-2 gap-4">
              ...
              {step === 1 && (
                <>
                  <Field label="Job Title"><input className={IC} style={IS} placeholder="e.g. Senior Engineer" value={form.role} onChange={e=>set("role",e.target.value)}/></Field>
                  <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Manager"><input className={IC} style={IS} placeholder="e.g. Devon Park" value={form.manager} onChange={e=>set("manager",e.target.value)}/></Field>
                  ...
                  <Field label="Employment Type">
                    <select className={IC} style={IS} value={form.empType} onChange={e=>set("empType",e.target.value)}>
                      <option>Regular</option><option>Contractual</option><option>Part-time</option><option>Custom</option>
                    </select>
                  </Field>
                </>
              )}
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Annual Salary ($)">
                      <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span><input type="number" className={IC} style={{...IS,paddingLeft:"1.75rem"}} placeholder="0" value={form.salary} onChange={e=>set("salary",e.target.value)}/></div>
                    </Field>
                  ...
                  <Field label="Benefits Package">
                    <select className={IC} style={IS} value={form.benefits} onChange={e=>set("benefits",e.target.value)}>
                      <option>Standard</option><option>Premium</option><option>None</option>
                    </select>
                  </Field>
                  {form.salary > 0 && (
                    <div className="rounded-lg p-4" style={{backgroundColor:"#111",border:"1px solid #1e1e1e"}}>
                      <p className="text-xs uppercase tracking-widest text-gray-600 mb-3" style={{fontFamily:"system-ui,sans-serif"}}>Estimated Per-Period Pay</p>
                      {(()=>{
                        const periods = form.payFreq==="Monthly"?12:form.payFreq==="Semi-monthly"?24:26;
                        const g = Number(form.salary)/periods;
                        return(
                          <div className="grid grid-cols-2 gap-3">
                            {[["Gross",`$${g.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}`,"#fff"],
                              ["Frequency",form.payFreq,"#aaa"]].map(([l,v,c])=>(
                              <div key={l}>
              ...
              {step === 3 && (
                <div className="space-y-4">
                  <div className="rounded-lg p-5" style={{backgroundColor:"#0d0d0d",border:"1px solid #1e1e1e"}}>
                    <p className="text-xs uppercase tracking-widest text-gray-500 mb-4" style={{fontFamily:"system-ui,sans-serif"}}>Summary</p>
                    {[
                      ["Full Name",   form.name],
                      ["Email",       form.email],
                      ["Phone",       form.phone||"—"],
                      ...
                      ["Start Date",  form.joined||"Today"],
                      ["Emp. Type",   form.empType],
                      ["Salary",      form.salary?`$${Number(form.salary).toLocaleString()}/yr`:"—"],
                      ["Pay Freq.",   form.payFreq],
                      ...
                      <div key={l} className="flex justify-between py-2" style={{borderBottom:"1px solid #1a1a1a"}}>
                        ...
                        <span className="text-gray-200 text-sm" style={{fontFamily:"system-ui,sans-serif"}}>{v}</span>
                      </div>
                    ))}
                  </div>
                  <div className="rounded-lg p-3" style={{backgroundColor:"#0a1a0a",border:"1px solid #1e3a1e"}}>
                    <p className="text-xs text-gray-400" style={{fontFamily:"system-ui,sans-serif"}}>
                      ✓ A welcome email will be sent to <strong className="text-white">{form.email}</strong> upon creation. Tax and statutory deduction settings can be configured in the Compensation tab after onboarding.
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="px-7 py-5 flex items-center justify-between flex-shrink-0" style={{borderTop:"1px solid #1a1a1a"}}>
              ...
              <button
                ...
                style={{fontFamily:"system-ui,sans-serif",backgroundColor:canNext[step]?"#fff":"#1a1a1a",color:canNext[step]?"#000":"#444",cursor:canNext[step]?"pointer":"not-allowed"}}>
                {step===STEPS.length-1?"Create Employee ✓":"Next →"}
              </button>
            </div>
          </>
        )}
